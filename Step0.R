...
data_dir <- "/Volumes/shiz-wm-netzero/users/yuqing/daiyy/GreenSpace/treated/data/"

my_list <- readRDS("/Volumes/shiz-wm-netzero/users/yuqing/daiyy/GreenSpace/treated/data/mydat_16T24NO23.rds")
...
add_labels_to_list <- function(my_list, labels_df, id_col = "person_id") {
  lapply(my_list, function(df) {
    df2 <- df
    # standardize ID column if needed
    if (!(id_col %in% names(df2))) {
      candidates <- c("person_id","personid","pid","id","subject_id","subject","participant_id","case_id")
    ...
    dplyr::left_join(df2, labels_df, by = setNames("person_id", id_col))
  })
}

# ==== Diagnostics if you got zero "improved" ====

stopifnot(exists("res"))  # res from classify_ndvi_groups(...)
lab <- res$labels
thr <- res$thresholds
prestable_n <- sum(lab$pre_stable %in% TRUE)
treated_n   <- sum(lab$ndvi_group == "improved", na.rm = TRUE)

message("Pre-stable units: ", prestable_n)
message("Improved units:   ", treated_n)
print(thr)

# Distribution among pre-stable only
library(dplyr)
prestable <- lab %>% filter(pre_stable)

if (nrow(prestable) > 0) {
  cat("\nΔ (post - pre) among pre-stable (quantiles):\n")
  print(quantile(prestable$delta, probs = c(.1,.25,.5,.75,.9), na.rm = TRUE))
  cat("\nPost-share-up among pre-stable (quantiles):\n")
  print(quantile(prestable$post_share_up, probs = c(.1,.25,.5,.75,.9), na.rm = TRUE))
  cat("\nYour cutoffs:\n")
  cat("  improvement_thr (Δ) :", thr$improvement_thr, "\n")
  cat("  noise_threshold (|Δ|):", thr$noise_threshold, "\n")
  cat("  persistence_margin m :", thr$persistence_margin, "\n")
  cat("  persistence required  :", thr$persistence_req, "\n")
  cat("  slope_threshold       :", thr$slope_threshold, "\n")
  cat("  sd_threshold          :", thr$sd_threshold, "\n")
} else {
  cat("\nNo pre-stable units: thresholds for slope/SD may be too tight.\n")
}
...
# person-level derived metrics
desc_df <- lab %>%
  mutate(
    baseline_pre = pre_mean,
    abs_change   = delta,
    rel_change   = 100 * delta / pmax(pre_mean, 1e-6)   # % vs baseline
  )

fmt_pm_sd   <- function(m, s) sprintf("%.3f ± %.3f", m, s)
fmt_pct_sd  <- function(m, s) sprintf("%.1f%% ± %.1f%%", m, s)

# ---- 3-group summary (improved / stable / ambiguous_unstable)
summary_3grp <- desc_df %>%
  group_by(ndvi_group) %>%
  summarise(
    n_persons    = n(),
    # Baseline (pre) and Post means across people
    baseline_mean = mean(baseline_pre, na.rm = TRUE),
    baseline_sd   = sd(baseline_pre,   na.rm = TRUE),
    post_mean     = mean(post_mean,    na.rm = TRUE),
    post_sd       = sd(post_mean,      na.rm = TRUE),
    
    # Absolute and relative changes across people
    delta_mean    = mean(abs_change,   na.rm = TRUE),
    delta_sd      = sd(abs_change,     na.rm = TRUE),
    rel_mean_pct  = mean(rel_change,   na.rm = TRUE),
    rel_sd_pct    = sd(rel_change,     na.rm = TRUE),
    
    # Robust (medians) for appendix
    baseline_median = median(baseline_pre, na.rm = TRUE),
    delta_median    = median(abs_change,   na.rm = TRUE),
    rel_median_pct  = median(rel_change,   na.rm = TRUE),
    
    # Persistence diagnostics
    post_share_up_mean   = mean(post_share_up,   na.rm = TRUE),
    post_share_down_mean = mean(post_share_down, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    baseline_mean_sd   = fmt_pm_sd(baseline_mean, baseline_sd),
    post_mean_mean_sd  = fmt_pm_sd(post_mean,     post_sd),
    delta_mean_sd      = fmt_pm_sd(delta_mean,    delta_sd),
    rel_mean_sd_pct    = fmt_pct_sd(rel_mean_pct, rel_sd_pct)
  ) %>%
  select(
    ndvi_group, n_persons,
    baseline_mean_sd, post_mean_mean_sd,
    delta_mean_sd, rel_mean_sd_pct,
    baseline_median, delta_median, rel_median_pct,
    post_share_up_mean, post_share_down_mean
  )

print(summary_3grp)
# write.csv(summary_3grp, "ndvi_summary_3groups.csv", row.names = FALSE)

# ---- Optional: 4-group summary (separating 'declined')
summary_4grp <- desc_df %>%
  group_by(ndvi_group4) %>%
  summarise(
    n_persons    = n(),
    baseline_mean = mean(baseline_pre, na.rm = TRUE),
    baseline_sd   = sd(baseline_pre,   na.rm = TRUE),
...
# print(summary_4grp)
# write.csv(summary_4grp, "ndvi_summary_4groups.csv", row.names = FALSE)
