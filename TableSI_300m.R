...
data_dir <- "/Volumes/shiz-wm-netzero/users/yuqing/daiyy/GreenSpace/data/A_used/"

my_list <- readRDS("/Volumes/shiz-wm-netzero/users/yuqing/daiyy/GreenSpace/data/A_used/mydat_16T24NO23.rds")
...
# ---- Table 1: baseline characteristics by NDVI_300m group (2016) ----

# install + load helper package for nice Table 1
pkgs <- c("dplyr", "gtsummary")
new_pkgs <- pkgs[!pkgs %in% installed.packages()[, "Package"]]
if (length(new_pkgs)) install.packages(new_pkgs, repos = "https://cloud.r-project.org")
library(dplyr)
library(gtsummary)

# 1) Take 2016 data and define greenness group
dat2016 <- my_list_labeled[["complete_2016_cleaned"]] %>%
  filter(year == 2016) %>%  # should already all be 2016, but safe
  mutate(
    # use the 300m greenness label – use .y if you have both x & y
    greenness_300 = label_300.y,
    greenness_300 = factor(
      greenness_300,
      levels = c("improved", "stable", "ambiguous")
    )
  ) %>%
  filter(!is.na(greenness_300))

# 2) Recode categorical variables (change labels if your coding differs!)
dat2016 <- dat2016 %>%
  mutate(
    hypertension = factor(hypertension, levels = c(0, 1), labels = c("No", "Yes")),
    diabetes     = factor(diabetes,     levels = c(0, 1), labels = c("No", "Yes")),
    smoking      = factor(smoking,      levels = c(0, 1), labels = c("No", "Yes")),
    # ⚠️ I don't know your sex coding; flip "Male"/"Female" if needed
    sex          = factor(sex,          levels = c(0, 1), labels = c("Female", "Male"))
  )

# 3) Build Table 1 (age, hypertension, diabetes, smoking, sex by greenness group)
tbl1 <- dat2016 %>%
  select(greenness_300, age, hypertension, diabetes, smoking, sex) %>%
  tbl_summary(
    by = greenness_300,
    statistic = list(
      all_continuous()  ~ "{mean} ({sd})",   # age as mean (SD)
      all_categorical() ~ "{n} ({p}%)"       # binary vars as n (%)
    ),
    missing = "no"
  ) %>%
  add_n() %>%        # adds total N per column
  add_p() %>%        # optional: p-values across greenness groups
  modify_header(label ~ "**Characteristic**") %>%
  modify_spanning_header(starts_with("stat_") ~ "**Neighbourhood greenness group (300 m)**") %>%
  bold_labels()

# print in RStudio / console
tbl1

# to Word (RTF)
tbl1 %>% as_flex_table() %>% flextable::save_as_docx(path = "/Volumes/shiz-wm-netzero/users/yuqing/daiyy/GreenSpace/fig/Table1_baseline_2016.docx")
