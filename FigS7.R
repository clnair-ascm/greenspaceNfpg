...
panel_fbg <- panel_fbg %>%
  mutate(
    treat_time = if_else(label_300 == "improved" & year >= 2020, 1L, 0L)
  ) %>%
  arrange(person_id, year)
...
gg_fig1 <- ggplot(
  ...
  # Intervention line
  geom_vline(xintercept = 2019, linetype = "dashed", colour = "grey60") +
  # 95% CI ribbon
  geom_ribbon(
...
ggsave(
  filename = "/Users/yqdai/Downloads/GreenSpace/fig/FBG_raw_traj_improved_vs_stable.tiff",
  plot     = gg_fig1,
  width    = 6,
  height   = 4.5,
  units    = "in",
...
ggsave(
  filename = "/Users/yqdai/Downloads/GreenSpace/fig/FBG_sdid_overall.tiff",
  plot     = gg_fig2,
  width    = 6,
  height   = 4.5,
  units    = "in",
...
# ------------------------------------------------------------
panel <- readRDS(
  "/Users/yqdai/Downloads/GreenSpace/data/D_all_SI/panel_14_24_R1000_DTO_SEV.rds"
)
...
panel_fbg <- panel_fbg %>%
  mutate(
    treat_time = if_else(label_1000 == "improved" & year >= 2020, 1L, 0L)
  ) %>%
  arrange(person_id, year)
